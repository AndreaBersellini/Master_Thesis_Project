...
from XShared.utils.dqn_conv import *
...
class AgentDQL(Agent):
    def __init__(self, iterations : dict, epsilon_min : float, epsilon_max : float, hyperparameters : dict, paths : dict, obs_space : int, act_space : int):
        ...
        self._policy_network = ImageDQN(3, self._obs_space, self._act_space, self._learning_rate, self._paths['policy_net'], self._paths['results']).to(self._device)
        torch.manual_seed(69)
        self._target_network = ImageDQN(3, self._obs_space, self._act_space, self._learning_rate, self._paths['target_net'], self._paths['results']).to(self._device)
    ...
    def _train_network(self) -> tuple:
        ...
        self._policy_network.train() # Set in training mode
        ...
        #print(f"States: {states.shape}, Actions: {actions.shape}, Next: {next_states.shape}, Rewards: {rewards.shape}")
        #print(f"QP shape: {q_pred.shape}")

        q_pred = q_pred.gather(dim=1, index=actions)

        """with torch.no_grad():
            target_pred : torch.Tensor = self._target_network(next_states)
            #target_pred : torch.Tensor = self._policy_network(next_states)
            q_best, _ = torch.max(input=target_pred, dim=1, keepdim=True)"""
        with torch.no_grad():
        ...
        #print(f"Q-Pred: {q_pred}, Q-Best: {q_best}, Q-Target: {q_target}")
        #time.sleep(1)
    ...
    def train(self, env : 'Environment') -> dict:
        ...
        for generation in range(self._generations):
            ...
            for episode in tqdm(range(self._episodes), "Gathering states"):
                ...
                state_memory = deque([state] * 3, maxlen=3)
                ...
                # --------------------------------
                if episode % 20 == 0 : self._target_network.load_state_dict(self._policy_network.state_dict())
                # --------------------------------
            ...
            if self._complete:
                if (4230 - 30) <= self._distance_upperbound and  self._distance_upperbound <= (4230 + 30):
                    break
            # --------------------------------
    ...
    def _plot_features(self, samples : int = 1):
        if len(self._replay_buffer) >= samples:

            states, _, _, _, _ = zip(*self._replay_buffer.batch(samples))

            states = torch.stack(states)

            with torch.no_grad():
                self._policy_network.features(states)
