...
class CustomEnv(Environment):
    ...
    def _observation(self) -> tuple:
        ...
        min_val = -sampling#-(self._range * 2)
        max_val = sampling#self._range * 2

        on_ground = 1.0

        prev_top_platf = (max_val, max_val)
        next_top_platf = (max_val, max_val)

        prev_bot_platf = (max_val, max_val)
        next_bot_platf = (max_val, max_val)

        prev_ground_x = max_val
        next_ground_x = max_val

        coin = (max_val, max_val)
        plant = (max_val, max_val)
        ...
        for player in self.game._arena.actors():
            ...
            if isinstance(player, Player):
                ...
                on_ground = 1.0 if player.velocity() == 0.0 else 0.0 # Already normalized
        ...
        couples = [prev_top_platf, next_top_platf, prev_bot_platf, next_bot_platf, coin, plant]
        distances = [prev_ground_x, next_ground_x] + [item for group in couples for item in group]

        #state = tuple([on_ground] + [((x - min_val)/(max_val - min_val)) for x in distances]) # Normalization [0, +1]
        state = tuple([on_ground] + [2 * ((x - min_val)/(max_val - min_val)) - 1 for x in distances]) # Normalization [-1, +1]

        #state = tuple([on_ground] + [x for x in distances])

        state = tuple(round(v, 4) for v in state)

        #print(state)
        #time.sleep(1)
        #state = tuple([on_ground] + distances)
        return state